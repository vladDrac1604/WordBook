  ...
  prepareSavePayload() {
    ...
    for (let x of this.definitions) {
      if (!speechPartsMain.includes(x.partOfSpeech)) {
        ...
        speechPartsArr.push({ type: x.partOfSpeech, description: x.text.replace(/<[^>]*>/g, '') });
      } else {
        const data = speechPartsArr.find((item: any) => item.type === x.partOfSpeech);
        data.description = data.description + " (OR) " + x.text.replace(/<[^>]*>/g, '');
      }
    }
    for (let x of this.examples) {
      if(examplesArr.length < 3) examplesArr.push({ description: x.text });
    }
